-- Admin tools: reset all sales / delete one transaction
-- Protected by administrator role + password 11310
...
-- Wipe all sales / cash / unpaid orders. Keeps products, users, categories, tables.
...
BEGIN
  ...
  -- Restore stock taken by paid sales (trigger will not fire on UPDATE)
...
-- Delete one paid invoice (transaction) and its order; restore stock + cash session
...
BEGIN
  ...
  -- Restore stock if linked order was paid
  ...
  -- Remove cash lines for this invoice, then detach
